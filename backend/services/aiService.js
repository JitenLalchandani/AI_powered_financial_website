...
// ─────────────────────────────────────────────────────────────────────────────
function callHuggingFace(apiKey, prompt, maxTokens = MAX_TOKENS, requiresJSON = false) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      inputs: prompt,
      parameters: {
...
//  Claude API (fallback)
// ─────────────────────────────────────────────────────────────────────────────
function callClaude(messages, systemPrompt, maxTokens = MAX_TOKENS, requiresJSON = false) {
  return new Promise((resolve, reject) => {
...
async function forecastCashFlow(user, Transaction) {
  ...
  const monthly = await Transaction.aggregate([
    { \: { user: user._id, date: { \: m6S } } },
    { \: { _id: { yr: { \: '' }, mo: { \: '' }, type: '' }, total: { \: '' } } },
    { \: { '_id.yr': 1, '_id.mo': 1 } }
  ]);

  const incomeMonths = monthly.filter(m => m._id.type === 'income');
  const expenseMonths = monthly.filter(m => m._id.type === 'expense');
  
  const avgMonthlyIncome = incomeMonths.length 
    ? incomeMonths.reduce((s, m) => s + m.total, 0) / incomeMonths.length 
    : 0;
  const avgMonthlyExpense = expenseMonths.length 
    ? expenseMonths.reduce((s, m) => s + m.total, 0) / expenseMonths.length 
    : 0;
  
  const net30 = Math.round(avgMonthlyIncome - avgMonthlyExpense);
  const net90 = Math.round((avgMonthlyIncome - avgMonthlyExpense) * 3);
  const confidence = monthly.length >= 6 ? 'high' : monthly.length >= 3 ? 'medium' : 'low';

  let summary = 'Based on your transaction data, your projected monthly net cash flow is approximately ₹' + net30.toLocaleString('en-IN') + '.';
  let risks = ['Limited historical data may affect accuracy'];
  let recommendations = ['Add more transactions across multiple months for better forecasting'];

  // Try AI for enhanced summary, but never fail if AI breaks
  try {
    const fmt = n => '₹' + Number(n || 0).toLocaleString('en-IN');
    const history = monthly.map(m => m._id.yr + '-' + String(m._id.mo).padStart(2, '0') + ' ' + m._id.type + ': ' + fmt(m.total)).join('
') || 'No data.';
    
    const sys = 'You are a financial forecasting expert for India. Analyse the monthly transaction history and return ONLY a JSON object — no markdown, no extra text. Required shape: { "risks": ["risk1"], "recommendations": ["action1"], "summary": "2-sentence summary" }';

    const raw = await callAI(
      [{ role: 'user', content: 'Monthly data:
' + history + '
...
Generate cash flow summary.' }],
    ...
    let jsonStr = raw.replace(/\\/gi, '').trim();
    ...
    console.error('[forecastCashFlow] AI enhancement failed, using local fallback:', e.message);
  }

  return {
    next30days: { expectedIncome: Math.round(avgMonthlyIncome), expectedExpenses: Math.round(avgMonthlyExpense), netCashFlow: net30, confidence },
    next90days: { expectedIncome: Math.round(avgMonthlyIncome * 3), expectedExpenses: Math.round(avgMonthlyExpense * 3), netCashFlow: net90, confidence },
    risks,
    recommendations,
    summary
  };
