...
router.post('/chat', async (req, res) => {
  try {
    ...
    const isKey = err.message.includes('API_KEY') || err.message.includes('quota') || err.message.includes('balance');
    return res.status(isKey ? 503 : 500).json({
      success: false,
      message: isKey
        ? 'AI quota exceeded. Please check your API keys or add credits.'
        : err.message
    });
  }
});
...
router.post('/analyse', async (req, res) => {
  try {
    ...
    if (count < 1) return badReq(res, 'Add at least 3 transactions first, then run an analysis.');

    const rawInsights = await aiService.generateInsights(req.user, Transaction);
    if (!rawInsights.length) {
      const existing = await require('../models/Insight').find({ user: req.user._id, dismissed: false }).sort({ priority: 1, createdAt: -1 }).limit(25);
      return ok(res, existing, { message: 'Analysis done — showing your existing insights.' });
    }
...
router.get('/forecast', async (req, res) => {
  try {
    const count = await Transaction.countDocuments({ user: req.user._id });
    if (count < 1) return badReq(res, 'Add at least 3 transactions to generate a forecast.');
    const forecast = await aiService.forecastCashFlow(req.user, Transaction);
    if (!forecast) return ok(res, { summary: 'Start adding transactions to see your cash flow forecast.', confidence: 'low', monthly: [] });
